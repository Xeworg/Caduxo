...
(Previously: the requirement additionally pinned the day grid,
day cells, today ring, selection ring, badge dots, month picker
overlay, year picker overlay, and day-detail panel to the
shared DaisyUI themed primitives. It did not yet open in annual
view; the default landing primitive was `<CalendarMonth />`
rendering a single month. It did not yet describe a page-level
year header, year chevrons, a `Today` button,
today's-anchor-across-years, or the year-grid keyboard surface.
The tile headers inside each mini-month tile remain read-only
per the `Calendar annual view` requirement; only the page-level
`viewYear` year-picker trigger in the page header is clickable.)
