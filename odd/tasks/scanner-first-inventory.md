...
- Work-unit commit: `642ef09 feat(scanner): clear selected lot when leaving Scanner tab`. The scanner-side consumer that clears `pinnedContext` on the sentinel arrived in the task 12 work-unit commit (`831ed15`), which completes the contract end-to-end.
...
- Work-unit commit: `831ed15 feat(scanner): promote lot operations to first-class modes`. Preceded on the same branch by `acf8f2a fix(ui): prevent modal-box from establishing containing block for popovers` (the prior Listbox popover fix recovered from the stash) and `642ef09 feat(scanner): clear selected lot when leaving Scanner tab` (the App/navigation side of the exit signal).
- Follow-up — lot-context entry points: review identified two behavioral gaps between the canonical first-class modes and the lot-context entry flows. After Dashboard/Calendar/Product Detail handoff the pinned lot landed on the default `sale` mode rather than `lot_context` (the navigation consumer never assigned `mode` after pinning), and a manual `lot_match` scanner resolution set `selectedLotId` for the Sale/Stock-out derivations but did not pin `pinnedContext`, leaving the three lot modes disabled even though a concrete lot was already active. Fixed both in a single tight diff: the navigation consumer now delegates to `applyResolveResult({ match_type: "lot_match", lot, product })` after fetching, which is the same single source of truth invoked by a manual scan resolution. This populates `resolved`, `selectedLotId`, `locationId`, and `pinnedContext` in one place so Venta/Salida/Registro operate on the pinned lot identically to a manual lot_match scan; `mode = "lot_context"` is then set as the Dashboard-handoff-only landing tab. Both writes share a single synchronous block so Svelte 5 batches them and the defensive reset cannot fire between them. Removed an unreferenced parallel location-lookup mechanism (`lotContextLocationById` and its populate effect) that the redo accidentally introduced; rendering already uses `resolveLocationDisplay` from `src/lib/lotDisplay.js` fed by `locationLookup`. Two blind adversarial reviewers (`jd-judge-a`, `jd-judge-b`) independently refuted an earlier fabricated diagnosis about Svelte 5 hoisting; their findings are recorded in the orchestrator memory. Independent verification: `npm run check` 0 errors / 0 warnings, `npm run build` ✓, `git diff --check` clean. Work-unit commit: `0112661 fix(scanner): make Venta/Salida/Registro work on Dashboard-handoff pinned lot`.
